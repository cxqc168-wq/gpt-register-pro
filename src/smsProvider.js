...
class SMSProvider {
    constructor(apiKey, proxy = null) {
        ...
        this.proxy = proxy;
        // HeroSMS 接口通过代理访问（国内直连 hero-sms.com 不稳定）
        this.axios = axios.create(buildAxiosProxyConfig(proxy));
        if (proxy?.host && proxy?.port) {
            console.log(`[SMS] 接口使用代理: ${proxy.host}:${proxy.port}`);
        }
    }
    ...
    /**
     * 账户余额（sms-activate 协议：ACCESS_BALANCE:balance 文本）
     * @returns {Promise<{balance: number|null, userId: number|null, username: string}>}
     */
    async getBalance() {
        const raw = await this.request('getBalance');
        const match = String(raw || '').match(/[-+]?\d+(?:\.\d+)?/);
        if (!match) throw new Error(`获取余额失败: ${String(raw).slice(0, 120)}`);
        return { balance: Number(match[0]), userId: null, username: '' };
    }
    ...
    async getNumber(service = 'dr', country = 16, maxRetries = 5, operator = '') {
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            ...
            try {
                ...
                // HeroSMS 对"无可用号码"返回 HTTP 404 + JSON {"title":"NO_NUMBERS"}
                // （sms-activate 协议原生为 HTTP 200 纯文本，这里兼容两种形态）
                ...
                if (isNoNumbers) {
                    console.log(`[SMS] 暂无可用号码${operator ? `（运营商 ${operator} 无库存）` : ''}，${attempt < maxRetries ? '3秒后重试...' : '已达最大重试次数'} (${attempt}/${maxRetries})`);
...
module.exports = { SMSProvider, buildProxyUrl, buildAxiosProxyConfig };
