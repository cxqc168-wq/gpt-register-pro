...
const { OutlookPool, OutlookMailClient } = require('./outlookProvider');
...
class MailProvider {
    constructor(options) {
        ...
        // Outlook 邮箱池：池实例可由外部注入（与 CLI 共享同一份存储），否则按配置文件自建
        if (this.provider === 'outlook') {
            this.outlookPool = options.outlookPool || new OutlookPool(options.outlookPoolFile);
            this.outlookClient = new OutlookMailClient(
                options.outlookUseProxy === false ? null : this.proxy
            );
            this.outlookAccount = null;
        }
        // 邮箱 Worker 通过代理访问（国内直接连 Cloudflare Worker 不稳定）
        this.axiosDirect = axios.create(buildAxiosProxyConfig(this.proxy));
        if (this.proxy?.host && this.proxy?.port) {
            console.log(`[Mail] 邮箱接口使用代理: ${this.proxy.host}:${this.proxy.port}`);
        }
    }
    ...
    async _createAddressOutlook() {
        const account = this.outlookPool.allocate();
        if (!account) {
            const error = new Error('[outlook] Outlook 账号池已耗尽（无可用邮箱），请在「Outlook 邮箱池」中导入新的卡密');
            error.code = 'OUTLOOK_POOL_EXHAUSTED';
            throw error;
        }
        ...
        console.log(`[Mail][outlook] 分配邮箱: ${this.address}`);
        return { jwt: this.jwt, address: this.address, addressId: null };
    }
    ...
    async createAddress(name = null) {
        ...
        // auto mode: 优先 legacy，不兼容时自动切换 cloud-mail
        try {
            ...
            console.warn('[Mail] legacy 邮箱接口不兼容，自动切换到 cloud-mail 协议');
    ...
    useExistingAddressSession(session = {}) {
        ...
        if (this.provider === 'outlook') {
            if (!address) {
                throw new Error('[outlook] session requires address');
            }
            const account = this.outlookPool.getAccount(address);
            if (!account) {
                throw new Error(`[outlook] mailbox ${address} not in pool`);
            }
            this.address = account.email;
            this.outlookAccount = account;
            this.jwt = 'outlook';
            console.log(`[Mail][outlook] reuse mailbox: ${this.address}`);
            return;
        }
        if (!address || !jwt) {
    ...
    /**
     * Outlook 绑定成功确认（仅 outlook provider 生效）
     */
    confirmOutlookBound(phone = null) {
        if (this.provider !== 'outlook' || !this.address) return;
        this.outlookPool.markUsed(this.address, phone);
    }

    /**
     * Outlook 分配回滚（仅 outlook provider 生效）
     */
    rollbackOutlookAllocation() {
        if (this.provider !== 'outlook' || !this.address) return;
        this.outlookPool.markAvailable(this.address);
    }

    /**
     * Outlook 邮箱标记无效（仅 outlook provider 生效）
     * 用于邮箱验证码已通过但后续流程失败的场景：该邮箱的账号可能已创建，不得回池复用
     */
    markOutlookInvalid(reason = '') {
        if (this.provider !== 'outlook' || !this.address) return;
        this.outlookPool.markInvalid(this.address, reason);
    }
    ...
    async _getMailsOutlook(address, limit = 5) {
        if (!address) {
            throw new Error('[outlook] current mailbox missing');
        }
        const account = this.outlookPool.getAccount(address);
        if (!account) {
            throw new Error(`[outlook] mailbox ${address} not in pool`);
        ...
        try {
            return await this.outlookClient.fetchMails(account, limit, {
                // OAuth 轮换出新 refresh_token 时回写账号池存储
                onTokenRotated: async (rotatedAccount) => {
                    const stored = this.outlookPool.getAccount(rotatedAccount.email);
                    if (stored) {
                        stored.refreshToken = rotatedAccount.refreshToken;
                        stored.updatedAt = Date.now();
                        this.outlookPool._persist();
                    }
                },
            });
        } catch (error) {
            // 仅 token 彻底失效（两种 scope 均不可用）才标记账号 invalid；网络/服务器错误不标记
            if (error?.tokenFailure) {
                this.outlookPool.markInvalid(address, error.message);
            }
            throw error;
        }
    }
    ...
    async getMails(limit = 10, offset = 0) {
        if (this.provider === 'outlook') {
            return await this._getMailsOutlook(this.address, limit);
        }
        if (this.provider === 'cloud-mail') {
    ...
    async getMailsByAddress(address, limit = 10, offset = 0) {
        if (this.provider === 'outlook') {
            return await this._getMailsOutlook(address, limit);
        }
        if (this.provider === 'cloud-mail') {
